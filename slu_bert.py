...
writer = SummaryWriter(log_dir='./logs_best')
...
model_name='bs'+str(args.batch_size)+'_lr'+str(args.lr)+'_dp'+str(args.dropout)+'.bin'
...
def decode(choice):
    ...
    with torch.no_grad():
        for i in range(0, len(dataset), args.batch_size):
            ...
            current_batch = from_example_list(args, tokenizer, cur_dataset, device, train=True)
...
if not args.testing:
    ...
    for i in range(args.max_epoch):
        ...
        scalar_dict = {'training_loss': epoch_loss / count, 'dev_loss': dev_loss, 'dev_acc': dev_acc,
                       'dev_fscore_p': dev_fscore['precision'], 'dev_fscore_recall': dev_fscore['recall'],
                       'dev_fscore_f': dev_fscore['fscore']}
        writer.add_scalars(main_tag='scalars', tag_scalar_dict=scalar_dict, global_step=(i + 1) * step_size)
